...
var import_vec39 = __toModule(require("@tldraw/vec"));
...
var TDShapeUtil = class extends import_core.TLShapeUtil {
  constructor() {
    ...
    this.getExpandedBounds = (shape) => {
      return import_core.Utils.expandBounds(this.getBounds(shape), this.bindingDistance);
    };
    this.getBindingPoint = (shape, fromShape, point, origin, direction, bindAnywhere) => {
      ...
      const intersection = intersections.sort((a, b) => import_vec.Vec.dist(b, origin) - import_vec.Vec.dist(a, origin))[0];
      const middlePoint = import_vec.Vec.med(point, intersection);
      let anchor;
      let distance;
      if (bindAnywhere) {
        anchor = import_vec.Vec.dist(point, center) < BINDING_DISTANCE / 2 ? center : point;
        distance = 0;
      } else {
        if (import_vec.Vec.distanceToLineSegment(point, middlePoint, center) < BINDING_DISTANCE / 2) {
          anchor = center;
        } else {
          anchor = middlePoint;
        }
        if (import_core.Utils.pointInBounds(point, bounds)) {
          ...
          distance = Math.max(this.bindingDistance, import_core.Utils.getBoundsSides(bounds).map((side) => import_vec.Vec.distanceToLineSegment(side[1][0], side[1][1], point)).sort((a, b) => a - b)[0]);
        }
      }
      const bindingPoint = import_vec.Vec.divV(import_vec.Vec.sub(anchor, [expandedBounds.minX, expandedBounds.minY]), [
...
PolygonUtils.leftSide = import_vec5.default.isLeft;
...
var TriangleUtil = class extends TDShapeUtil {
  constructor() {
    ...
    this.canClone = true;
    this.getShape = (props) => {
      return import_core5.Utils.deepMerge({
        id: "id",
    ...
    this.Component = TDShapeUtil.Component(({ shape, isBinding, isSelected, isGhost, meta, events }, ref) => {
      ...
      if (style.dash === DashStyle.Draw) {
        ...
        return /* @__PURE__ */ React3.createElement(import_core5.SVGContainer, __spreadValues({
          ref,
          id: shape.id + "_svg"
        }, events), isBinding && /* @__PURE__ */ React3.createElement("polygon", {
          className: "tl-binding-indicator",
      ...
      const sides = import_core5.Utils.pointsToLineSegments(points, true);
      const paths = sides.map(([start, end], i) => {
        const { strokeDasharray, strokeDashoffset } = import_core5.Utils.getPerfectDashProps(import_vec6.default.dist(start, end), strokeWidth * 1.618, shape.style.dash);
    ...
    this.Indicator = TDShapeUtil.Indicator(({ shape }) => {
      ...
      const styles2 = getShapeStyle(style, false);
      const sw = styles2.strokeWidth;
      return /* @__PURE__ */ React3.createElement("polygon", {
    ...
    this.getExpandedBounds = (shape) => {
      return import_core5.Utils.getBoundsFromPoints(getTrianglePoints(shape, this.bindingDistance).map((pt) => import_vec6.default.add(pt, shape.point)));
    };
    this.hitTestLineSegment = (shape, A, B) => {
    ...
    this.hitTestBounds = (shape, bounds) => {
      return import_core5.Utils.boundsContained(this.getBounds(shape), bounds) || (0, import_intersect3.intersectBoundsPolygon)(bounds, this.getPoints(shape)).length > 0;
    };
    this.getBindingPoint = (shape, fromShape, point, origin, direction, bindAnywhere) => {
      const expandedBounds = this.getExpandedBounds(shape);
      if (!import_core5.Utils.pointInBounds(point, expandedBounds))
        return;
      const points = getTrianglePoints(shape).map((pt) => import_vec6.default.add(pt, shape.point));
      const expandedPoints = getTrianglePoints(shape, this.bindingDistance).map((pt) => import_vec6.default.add(pt, shape.point));
      const closestDistanceToEdge = import_core5.Utils.pointsToLineSegments(points, true).map(([a, b]) => import_vec6.default.distanceToLineSegment(a, b, point)).sort((a, b) => a - b)[0];
      if (!(import_core5.Utils.pointInPolygon(point, expandedPoints) || closestDistanceToEdge < this.bindingDistance))
        return;
      const intersections = import_core5.Utils.pointsToLineSegments(expandedPoints.concat([expandedPoints[0]])).map((segment) => (0, import_intersect3.intersectRayLineSegment)(origin, direction, segment[0], segment[1])).filter((intersection2) => intersection2.didIntersect).flatMap((intersection2) => intersection2.points);
      if (!intersections.length)
        return;
      const center = import_vec6.default.add(getTriangleCentroid(shape), shape.point);
      const intersection = intersections.sort((a, b) => import_vec6.default.dist(b, origin) - import_vec6.default.dist(a, origin))[0];
      const middlePoint = import_vec6.default.med(point, intersection);
      let anchor;
      let distance;
      if (bindAnywhere) {
        anchor = import_vec6.default.dist(point, center) < BINDING_DISTANCE / 2 ? center : point;
        distance = 0;
      } else {
        if (import_vec6.default.distanceToLineSegment(point, middlePoint, center) < BINDING_DISTANCE / 2) {
          anchor = center;
        } else {
          anchor = middlePoint;
        }
        if (import_core5.Utils.pointInPolygon(point, points)) {
          distance = this.bindingDistance;
        } else {
          distance = Math.max(this.bindingDistance, closestDistanceToEdge);
        }
      }
      const bindingPoint = import_vec6.default.divV(import_vec6.default.sub(anchor, [expandedBounds.minX, expandedBounds.minY]), [
        expandedBounds.width,
        expandedBounds.height
      ]);
      return {
        point: import_vec6.default.clampV(bindingPoint, 0, 1),
        distance
      };
    };
    this.transform = transformRectangle;
  ...
  getPoints(shape) {
    ...
    return [
      ...
      [x, y + h],
      [x + w, y + h]
    ].map((pt) => import_vec6.default.rotWith(pt, this.getCenter(shape), rotation));
  }
};
...
function getTriangleDrawPoints(shape) {
  ...
  const lines = import_core5.Utils.rotateArray([
...
function getTriangleIndicatorPathTDSnapshot(shape) {
  ...
  return import_core5.Utils.getSvgPathFromStroke(strokePoints.map((pt) => pt.point.slice(0, 2)), false);
}
...
function getEllipseStrokePoints(shape, boundsCenter) {
  ...
  const center = import_vec7.default.sub(boundsCenter, point);
  const rx = radiusX + getRandom() * strokeWidth * 2;
  const ry = radiusY + getRandom() * strokeWidth * 2;
  const perimeter = import_core6.Utils.perimeterOfEllipse(rx, ry);
  const points = [];
...
function getEllipsePath(shape, boundsCenter) {
  ...
  const ry = radiusY + getRandom() * strokeWidth * 2;
  const perimeter = import_core6.Utils.perimeterOfEllipse(rx, ry);
  return import_core6.Utils.getSvgPathFromStroke((0, import_perfect_freehand3.getStrokeOutlinePoints)(getEllipseStrokePoints(shape, boundsCenter), {
...
function getEllipseIndicatorPathTDSnapshot(shape, boundsCenter) {
  return import_core6.Utils.getSvgPathFromStroke(getEllipseStrokePoints(shape, boundsCenter).map((pt) => pt.point.slice(0, 2)), false);
}
...
var EllipseUtil = class extends TDShapeUtil {
  constructor() {
    ...
    this.canBind = true;
    this.getShape = (props) => {
      return import_core7.Utils.deepMerge({
        id: "id",
    ...
    this.Component = TDShapeUtil.Component(({ shape, isGhost, isSelected, isBinding, meta, events }, ref) => {
      ...
      if (style.dash === DashStyle.Draw) {
        ...
        return /* @__PURE__ */ React4.createElement(import_core7.SVGContainer, __spreadValues({
          ref,
          id: shape.id + "_svg"
        }, events), isBinding && /* @__PURE__ */ React4.createElement("ellipse", {
          className: "tl-binding-indicator",
          cx: radiusX,
          cy: radiusY,
          rx,
      ...
      const perimeter = import_core7.Utils.perimeterOfEllipse(rx, ry);
      const { strokeDasharray, strokeDashoffset } = import_core7.Utils.getPerfectDashProps(perimeter < 64 ? perimeter * 2 : perimeter, strokeWidth * 1.618, shape.style.dash, 4);
      return /* @__PURE__ */ React4.createElement(import_core7.SVGContainer, __spreadValues({
        ref,
        id: shape.id + "_svg"
      }, events), isBinding && /* @__PURE__ */ React4.createElement("ellipse", {
    ...
    this.Indicator = TDShapeUtil.Indicator(({ shape }) => {
      ...
      const rx = Math.max(0, radiusX - sw / 2);
      const ry = Math.max(0, radiusY - sw / 2);
      return style.dash === DashStyle.Draw ? /* @__PURE__ */ React4.createElement("path", {
        d: getEllipseIndicatorPathTDSnapshot(shape, this.getCenter(shape))
      }) : /* @__PURE__ */ React4.createElement("ellipse", {
        cx: radiusX,
        cy: radiusY,
    ...
    this.hitTestPoint = (shape, point) => {
      return import_core7.Utils.pointInBounds(point, this.getRotatedBounds(shape)) && import_core7.Utils.pointInEllipse(point, this.getCenter(shape), shape.radius[0], shape.radius[1], shape.rotation || 0);
    };
    this.hitTestLineSegment = (shape, A, B) => {
    ...
    this.getBounds = (shape) => {
      return import_core7.Utils.getFromCache(this.boundsCache, shape, () => {
        return import_core7.Utils.getRotatedEllipseBounds(shape.point[0], shape.point[1], shape.radius[0], shape.radius[1], 0);
      });
    };
    this.getRotatedBounds = (shape) => {
      return import_core7.Utils.getRotatedEllipseBounds(shape.point[0], shape.point[1], shape.radius[0], shape.radius[1], shape.rotation);
    };
    this.hitTestBounds = (shape, bounds) => {
      const shapeBounds = this.getBounds(shape);
      return import_core7.Utils.boundsContained(shapeBounds, bounds) || (0, import_intersect4.intersectEllipseBounds)(this.getCenter(shape), shape.radius[0], shape.radius[1], shape.rotation || 0, bounds).length > 0;
    };
    this.shouldRender = (prev, next) => {
      return next.radius !== prev.radius || next.style !== prev.style;
    };
    this.getCenter = (shape) => {
      return import_vec8.Vec.add(shape.point, shape.radius);
    };
    this.getBindingPoint = (shape, fromShape, point, origin, direction, bindAnywhere) => {
      {
        const expandedBounds = this.getExpandedBounds(shape);
        ...
        let bindingPoint;
        let distance;
        if (!import_core7.Utils.pointInEllipse(point, center, shape.radius[0] + this.bindingDistance, shape.radius[1] + this.bindingDistance))
          return;
        if (bindAnywhere) {
          if (import_vec8.Vec.dist(point, this.getCenter(shape)) < 12) {
            bindingPoint = [0.5, 0.5];
          } else {
            bindingPoint = import_vec8.Vec.divV(import_vec8.Vec.sub(point, [expandedBounds.minX, expandedBounds.minY]), [
              expandedBounds.width,
              expandedBounds.height
            ]);
          }
          ...
          let intersection = (0, import_intersect4.intersectRayEllipse)(origin, direction, center, shape.radius[0], shape.radius[1], shape.rotation || 0).points.sort((a, b) => import_vec8.Vec.dist(a, origin) - import_vec8.Vec.dist(b, origin))[0];
          if (!intersection) {
            intersection = (0, import_intersect4.intersectLineSegmentEllipse)(point, center, center, shape.radius[0], shape.radius[1], shape.rotation || 0).points.sort((a, b) => import_vec8.Vec.dist(a, point) - import_vec8.Vec.dist(b, point))[0];
          }
          if (!intersection) {
            return void 0;
          }
          const anchor = import_vec8.Vec.med(point, intersection);
          if (import_vec8.Vec.distanceToLineSegment(point, anchor, this.getCenter(shape)) < 12) {
            bindingPoint = [0.5, 0.5];
          } else {
            bindingPoint = import_vec8.Vec.divV(import_vec8.Vec.sub(anchor, [expandedBounds.minX, expandedBounds.minY]), [
              expandedBounds.width,
              expandedBounds.height
            ]);
          }
          if (import_core7.Utils.pointInEllipse(point, center, shape.radius[0], shape.radius[1], shape.rotation || 0)) {
            ...
            distance = Math.max(this.bindingDistance / 2, import_vec8.Vec.dist(point, innerIntersection));
          }
        }
        return {
          point: bindingPoint,
    ...
    this.transformSingle = (shape, bounds) => {
      return {
        ...
        radius: import_vec8.Vec.div([bounds.width, bounds.height], 2)
      };
    };
  }
};
...
function getBendPoint(handles, bend) {
  ...
  const dist = import_vec9.default.dist(start.point, end.point);
  const midPoint = import_vec9.default.med(start.point, end.point);
  const bendDist = dist / 2 * bend;
  const u = import_vec9.default.uni(import_vec9.default.vec(start.point, end.point));
  const point = import_vec9.default.toFixed(Math.abs(bendDist) < 10 ? midPoint : import_vec9.default.add(midPoint, import_vec9.default.mul(import_vec9.default.per(u), bendDist)));
  return point;
}
function renderFreehandArrowShaft(shape) {
  ...
  const getRandom = import_core8.Utils.rng(id);
  const strokeWidth = getShapeStyle(style).strokeWidth;
  const startPoint = ((_a = shape.decorations) == null ? void 0 : _a.start) ? import_vec9.default.nudge(start.point, end.point, strokeWidth) : start.point;
  const endPoint = ((_b = shape.decorations) == null ? void 0 : _b.end) ? import_vec9.default.nudge(end.point, start.point, strokeWidth) : end.point;
  ...
  const path = import_core8.Utils.getSvgPathFromStroke(stroke);
  return path;
}
function renderCurvedFreehandArrowShaft(shape, circle, length, easing) {
  ...
  const startPoint = ((_a = shape.decorations) == null ? void 0 : _a.start) ? import_vec9.default.rotWith(start.point, center, strokeWidth / length) : start.point;
  const endPoint = ((_b = shape.decorations) == null ? void 0 : _b.end) ? import_vec9.default.rotWith(end.point, center, -(strokeWidth / length)) : end.point;
  const startAngle = import_vec9.default.angle(center, startPoint);
  const endAngle = import_vec9.default.angle(center, endPoint);
  const points = [];
  const count = 8 + Math.floor(Math.abs(length) / 20 * 1 + getRandom() / 2);
  for (let i = 0; i < count; i++) {
    const t = easing(i / count);
    const angle = import_core8.Utils.lerpAngles(startAngle, endAngle, t);
    points.push(import_vec9.default.toFixed(import_vec9.default.nudgeAtAngle(center, angle, radius)));
  }
...
function getCtp(shape) {
  ...
  return import_core8.Utils.circleFromThreePoints(start.point, end.point, bend.point);
}
function getArrowArc(shape) {
  const { start, end, bend } = shape.handles;
  const [cx, cy, radius] = import_core8.Utils.circleFromThreePoints(start.point, end.point, bend.point);
  const center = [cx, cy];
...
function getCurvedArrowHeadPoints(A, r1, C, r2, sweep) {
  ...
  const left = int ? import_vec9.default.nudge(import_vec9.default.rotWith(int, A, Math.PI / 6), A, r1 * -0.382) : A;
  const right = int ? import_vec9.default.nudge(import_vec9.default.rotWith(int, A, -Math.PI / 6), A, r1 * -0.382) : A;
  return { left, right };
}
function getStraightArrowHeadPoints(A, B, r) {
  ...
  const int = ints[0];
  const left = int ? import_vec9.default.rotWith(int, A, Math.PI / 6) : A;
  const right = int ? import_vec9.default.rotWith(int, A, -Math.PI / 6) : A;
  return { left, right };
}
...
function getArrowPath(shape) {
  ...
  const arrowDist = import_vec9.default.dist(start.point, end.point);
  const arrowHeadLength = Math.min(arrowDist / 3, strokeWidth * 8);
  const path = [];
  const isStraightLine = import_vec9.default.dist(_bend.point, import_vec9.default.toFixed(import_vec9.default.med(start.point, end.point))) < 1;
  if (isStraightLine) {
...
function getArcPoints(shape) {
  ...
  if (import_vec9.default.dist2(bend.point, import_vec9.default.med(start.point, end.point)) > 4) {
    const points = [];
    const { center, radius } = getArrowArc(shape);
    const startAngle = import_vec9.default.angle(center, start.point);
    const endAngle = import_vec9.default.angle(center, end.point);
    for (let i = 1 / 20; i < 1; i += 1 / 20) {
      const angle = import_core8.Utils.lerpAngles(startAngle, endAngle, i);
      points.push(import_vec9.default.nudgeAtAngle(center, angle, radius));
    }
    return points;
  } else {
...
function getArcLength(C, r, A, B) {
  const sweep = import_core8.Utils.getSweep(C, A, B);
...
var ArrowUtil = class extends TDShapeUtil {
  constructor() {
    ...
    this.Component = TDShapeUtil.Component(({ shape, isGhost, meta, events }, ref) => {
      ...
      const isStraightLine = import_vec10.Vec.dist(bend.point, import_vec10.Vec.toFixed(import_vec10.Vec.med(start.point, end.point))) < 1;
      const styles2 = getShapeStyle(style, meta.isDarkMode);
      const { strokeWidth } = styles2;
      const arrowDist = import_vec10.Vec.dist(start.point, end.point);
      const arrowHeadLength = Math.min(arrowDist / 3, strokeWidth * 8);
      const sw = 1 + strokeWidth * 1.618;
      let shaftPath;
      let startArrowHead;
      let endArrowHead;
      const getRandom = import_core9.Utils.rng(shape.id);
      const easing = EASINGS[getRandom() > 0 ? "easeInOutSine" : "easeInOutCubic"];
      if (isStraightLine) {
        const path = isDraw ? renderFreehandArrowShaft(shape) : "M" + import_vec10.Vec.toFixed(start.point) + "L" + import_vec10.Vec.toFixed(end.point);
        const { strokeDasharray, strokeDashoffset } = import_core9.Utils.getPerfectDashProps(arrowDist, strokeWidth * 1.618, shape.style.dash, 2, false);
        ...
        shaftPath = arrowDist > 2 ? /* @__PURE__ */ React5.createElement(React5.Fragment, null, /* @__PURE__ */ React5.createElement("path", {
        ...
        const { strokeDasharray, strokeDashoffset } = import_core9.Utils.getPerfectDashProps(Math.abs(length), sw, shape.style.dash, 2, false);
        if (decorations.start) {
        ...
        shaftPath = /* @__PURE__ */ React5.createElement(React5.Fragment, null, /* @__PURE__ */ React5.createElement("path", {
          className: "tl-stroke-hitarea",
          d: path
        }), /* @__PURE__ */ React5.createElement("path", {
          d: path,
      ...
      return /* @__PURE__ */ React5.createElement(import_core9.SVGContainer, __spreadValues({
        ref,
        id: shape.id + "_svg"
      }, events), /* @__PURE__ */ React5.createElement("g", {
        pointerEvents: "none",
        opacity: isGhost ? GHOSTED_OPACITY : 1
      }, shaftPath, startArrowHead && /* @__PURE__ */ React5.createElement(React5.Fragment, null, /* @__PURE__ */ React5.createElement("path", {
        className: "tl-stroke-hitarea",
        ...
        strokeLinejoin: "round",
        pointerEvents: "none"
      })), endArrowHead && /* @__PURE__ */ React5.createElement(React5.Fragment, null, /* @__PURE__ */ React5.createElement("path", {
        className: "tl-stroke-hitarea",
    ...
    this.getBounds = (shape) => {
      const bounds = import_core9.Utils.getFromCache(this.boundsCache, shape, () => {
        return import_core9.Utils.getBoundsFromPoints(getArcPoints(shape));
      });
      return import_core9.Utils.translateBounds(bounds, shape.point);
    };
    this.getRotatedBounds = (shape) => {
      let points = getArcPoints(shape);
      const { minX, minY, maxX, maxY } = import_core9.Utils.getBoundsFromPoints(points);
      if (shape.rotation !== 0) {
        points = points.map((pt) => import_vec10.Vec.rotWith(pt, [(minX + maxX) / 2, (minY + maxY) / 2], shape.rotation || 0));
      }
      return import_core9.Utils.translateBounds(import_core9.Utils.getBoundsFromPoints(points), shape.point);
    };
    this.getCenter = (shape) => {
      const { start, end } = shape.handles;
      return import_vec10.Vec.add(shape.point, import_vec10.Vec.med(start.point, end.point));
    };
    this.shouldRender = (prev, next) => {
      return next.decorations !== prev.decorations || next.handles !== prev.handles || next.style !== prev.style;
    };
    this.hitTestPoint = (shape, point) => {
      const pt = import_vec10.Vec.sub(point, shape.point);
      const points = getArcPoints(shape);
      for (let i = 1; i < points.length; i++) {
        if (import_vec10.Vec.distanceToLineSegment(points[i - 1], points[i], pt) < 1) {
          return true;
        }
      }
      return false;
    ...
    this.hitTestLineSegment = (shape, A, B) => {
      const ptA = import_vec10.Vec.sub(A, shape.point);
      const ptB = import_vec10.Vec.sub(B, shape.point);
      const points = getArcPoints(shape);
      for (let i = 1; i < points.length; i++) {
    ...
    this.hitTestBounds = (shape, bounds) => {
      const { start, end, bend } = shape.handles;
      const sp = import_vec10.Vec.add(shape.point, start.point);
      const ep = import_vec10.Vec.add(shape.point, end.point);
      if (import_core9.Utils.pointInBounds(sp, bounds) || import_core9.Utils.pointInBounds(ep, bounds)) {
        return true;
      }
      if (import_vec10.Vec.isEqual(import_vec10.Vec.med(start.point, end.point), bend.point)) {
        return (0, import_intersect6.intersectLineSegmentBounds)(sp, ep, bounds).length > 0;
      } else {
        const [cx, cy, r] = getCtp(shape);
        const cp = import_vec10.Vec.add(shape.point, [cx, cy]);
    ...
    this.transform = (shape, bounds, { initialShape, scaleX, scaleY }) => {
      ...
      const dist = import_vec10.Vec.dist(start.point, end.point);
      const midPoint = import_vec10.Vec.med(start.point, end.point);
      const bendDist = dist / 2 * initialShape.bend;
      const u = import_vec10.Vec.uni(import_vec10.Vec.vec(start.point, end.point));
      const point = import_vec10.Vec.add(midPoint, import_vec10.Vec.mul(import_vec10.Vec.per(u), bendDist));
      nextHandles["bend"] = __spreadProps(__spreadValues({}, bend), {
        point: import_vec10.Vec.toFixed(Math.abs(bendDist) < 10 ? midPoint : point)
      });
      return {
    ...
    this.onBindingChange = (shape, binding, target, targetBounds, expandedBounds, center) => {
      ...
      let handlePoint = import_vec10.Vec.sub(import_vec10.Vec.add([expandedBounds.minX, expandedBounds.minY], import_vec10.Vec.mulV([expandedBounds.width, expandedBounds.height], import_vec10.Vec.rotWith(binding.point, [0.5, 0.5], target.rotation || 0))), shape.point);
      if (binding.distance) {
        const intersectBounds = import_core9.Utils.expandBounds(targetBounds, binding.distance);
        const origin = import_vec10.Vec.add(shape.point, shape.handles[handle.id === "start" ? "end" : "start"].point);
        const direction = import_vec10.Vec.uni(import_vec10.Vec.sub(import_vec10.Vec.add(handlePoint, shape.point), origin));
        if (target.type === TDShapeType.Ellipse) {
          const hits = (0, import_intersect6.intersectRayEllipse)(origin, direction, center, target.radius[0] + binding.distance, target.radius[1] + binding.distance, target.rotation || 0).points.sort((a, b) => import_vec10.Vec.dist(a, origin) - import_vec10.Vec.dist(b, origin));
          if (hits[0]) {
            handlePoint = import_vec10.Vec.sub(hits[0], shape.point);
          }
        } else if (target.type === TDShapeType.Triangle) {
          const points = getTrianglePoints(target, BINDING_DISTANCE, target.rotation).map((pt) => import_vec10.Vec.add(pt, target.point));
          const segments = import_core9.Utils.pointsToLineSegments(points, true);
          const hits = segments.map((segment) => (0, import_intersect6.intersectRayLineSegment)(origin, direction, segment[0], segment[1])).filter((intersection) => intersection.didIntersect).flatMap((intersection) => intersection.points).sort((a, b) => import_vec10.Vec.dist(a, origin) - import_vec10.Vec.dist(b, origin));
          if (hits[0]) {
            handlePoint = import_vec10.Vec.sub(hits[0], shape.point);
          }
        } else {
          let hits = (0, import_intersect6.intersectRayBounds)(origin, direction, intersectBounds, target.rotation).filter((int) => int.didIntersect).map((int) => int.points[0]).sort((a, b) => import_vec10.Vec.dist(a, origin) - import_vec10.Vec.dist(b, origin));
          if (hits.length < 2) {
            hits = (0, import_intersect6.intersectRayBounds)(origin, import_vec10.Vec.neg(direction), intersectBounds).filter((int) => int.didIntersect).map((int) => int.points[0]).sort((a, b) => import_vec10.Vec.dist(a, origin) - import_vec10.Vec.dist(b, origin));
          }
          if (hits[0]) {
            handlePoint = import_vec10.Vec.sub(hits[0], shape.point);
          }
      ...
      return this.onHandleChange(shape, {
        [handle.id]: __spreadProps(__spreadValues({}, handle), {
          point: import_vec10.Vec.toFixed(handlePoint)
        })
      });
    };
    this.onHandleChange = (shape, handles) => {
      let nextHandles = import_core9.Utils.deepMerge(shape.handles, handles);
      ...
      nextHandles = __spreadProps(__spreadValues({}, nextHandles), {
        start: __spreadProps(__spreadValues({}, nextHandles.start), {
          point: import_vec10.Vec.toFixed(nextHandles.start.point)
        }),
        end: __spreadProps(__spreadValues({}, nextHandles.end), {
          point: import_vec10.Vec.toFixed(nextHandles.end.point)
        })
      });
      if ("bend" in handles) {
        const { start, end, bend } = nextHandles;
        const distance = import_vec10.Vec.dist(start.point, end.point);
        const midPoint = import_vec10.Vec.med(start.point, end.point);
        const angle = import_vec10.Vec.angle(start.point, end.point);
        const u = import_vec10.Vec.uni(import_vec10.Vec.vec(start.point, end.point));
        const ap = import_vec10.Vec.add(midPoint, import_vec10.Vec.mul(import_vec10.Vec.per(u), distance / 2));
        const bp = import_vec10.Vec.sub(midPoint, import_vec10.Vec.mul(import_vec10.Vec.per(u), distance / 2));
        const bendPoint = import_vec10.Vec.nearestPointOnLineSegment(ap, bp, bend.point, true);
        const bendDist = import_vec10.Vec.dist(midPoint, bendPoint);
        nextBend = import_core9.Utils.clamp(bendDist / (distance / 2), -0.99, 0.99);
        const angleToBend = import_vec10.Vec.angle(start.point, bendPoint);
        if (import_vec10.Vec.isEqual(midPoint, getBendPoint(nextHandles, nextBend))) {
          nextBend = 0;
      ...
      const offset = import_vec10.Vec.sub([nextBounds.minX, nextBounds.minY], topLeft);
      if (!import_vec10.Vec.isEqual(offset, [0, 0])) {
        Object.values(nextShape.handles).forEach((handle) => {
          handle.point = import_vec10.Vec.toFixed(import_vec10.Vec.sub(handle.point, offset));
        });
        nextShape.point = import_vec10.Vec.toFixed(import_vec10.Vec.add(nextShape.point, offset));
      }
      return nextShape;
    };
  }
...
var GroupUtil = class extends TDShapeUtil {
  constructor() {
    ...
    this.Component = TDShapeUtil.Component(({ shape, isBinding, isGhost, isHovered, isSelected, events }, ref) => {
      ...
      return /* @__PURE__ */ React6.createElement(import_core10.SVGContainer, __spreadValues({
        ref
      }, events), isBinding && /* @__PURE__ */ React6.createElement("rect", {
        className: "tl-binding-indicator",
        strokeWidth: this.bindingDistance
        ...
        fill: "transparent",
        pointerEvents: "all"
      }), /* @__PURE__ */ React6.createElement(ScaledLines, {
    ...
    this.Indicator = TDShapeUtil.Indicator(({ shape }) => {
      ...
      return /* @__PURE__ */ React6.createElement(ScaledLines, {
        strokeLinecap: "round",
        pointerEvents: "stroke"
      }, paths);
    });
...
var StickyUtil = class extends TDShapeUtil {
  constructor() {
    ...
    this.Component = TDShapeUtil.Component(({ shape, meta, events, isGhost, isBinding, isEditing, onShapeBlur, onShapeChange }, ref) => {
      ...
      const rContainer = React7.useRef(null);
      const rTextArea = React7.useRef(null);
      const rText = React7.useRef(null);
      const rIsMounted = React7.useRef(false);
      const handlePointerDown = React7.useCallback((e) => {
        e.stopPropagation();
      }, []);
      const handleTextChange = React7.useCallback((e) => {
      ...
      return /* @__PURE__ */ React7.createElement(import_core11.HTMLContainer, __spreadValues({
        ref
      }, events), /* @__PURE__ */ React7.createElement(StyledStickyContainer, {
        ref: rContainer,
      ...
      }), /* @__PURE__ */ React7.createElement(StyledText, {
        ref: rText,
        isEditing,
        alignment: shape.style.textAlign
      }, shape.text, "\u200B"), isEditing && /* @__PURE__ */ React7.createElement(StyledTextArea, {
        ref: rTextArea,
    ...
    this.transform = (shape, bounds, { scaleX, scaleY, transformOrigin }) => {
      const point = import_vec11.Vec.toFixed([
        bounds.minX + (bounds.width - shape.size[0]) * (scaleX < 0 ? 1 - transformOrigin[0] : transformOrigin[0]),
        bounds.minY + (bounds.height - shape.size[1]) * (scaleY < 0 ? 1 - transformOrigin[1] : transformOrigin[1])
      ]);
      return {
...
var TextUtil = class extends TDShapeUtil {
  constructor() {
    ...
    this.Component = TDShapeUtil.Component(({ shape, isBinding, isGhost, isEditing, onShapeBlur, onShapeChange, meta, events }, ref) => {
      const rInput = React8.useRef(null);
      const { text, style } = shape;
      const styles2 = getShapeStyle(style, meta.isDarkMode);
      const font = getFontStyle(shape.style);
      const rIsMounted = React8.useRef(false);
      const handleChange = React8.useCallback((e) => {
        let delta = [0, 0];
        ...
        switch (shape.style.textAlign) {
          ...
          case AlignStyle.Middle: {
            ...
            delta = import_vec12.Vec.div([nextBounds.width - currentBounds.width, 0], 2);
            break;
          }
          case AlignStyle.End: {
            const nextBounds = this.getBounds(__spreadProps(__spreadValues({}, shape), {
        ...
        onShapeChange == null ? void 0 : onShapeChange(__spreadProps(__spreadValues({}, shape), {
          point: import_vec12.Vec.sub(shape.point, delta),
          text: TLDR.normalizeText(e.currentTarget.value)
        }));
      }, [shape]);
      ...
      const handleBlur = React8.useCallback((e) => {
        e.currentTarget.setSelectionRange(0, 0);
        onShapeBlur == null ? void 0 : onShapeBlur();
      }, []);
      const handleFocus = React8.useCallback((e) => {
        if (!isEditing)
          return;
        if (!rIsMounted.current)
          return;
      ...
      return /* @__PURE__ */ React8.createElement(import_core12.HTMLContainer, __spreadValues({
        ref
      }, events), /* @__PURE__ */ React8.createElement(Wrapper, {
        isGhost,
        isEditing,
        onPointerDown: handlePointerDown
      }, /* @__PURE__ */ React8.createElement(InnerWrapper, {
        style: {
          font,
          color: styles2.stroke,
          textAlign: getTextAlign(style.textAlign)
        }
      }, isBinding && /* @__PURE__ */ React8.createElement("div", {
        className: "tl-binding-indicator",
        style: {
          position: "absolute",
          top: -this.bindingDistance,
      ...
      }), isEditing ? /* @__PURE__ */ React8.createElement(TextArea, {
        ref: rInput,
        style: {
          font,
          color: styles2.stroke
    ...
    this.getBounds = (shape) => {
      const bounds = import_core12.Utils.getFromCache(this.boundsCache, shape, () => {
        if (!melm) {
      ...
      return import_core12.Utils.translateBounds(bounds, shape.point);
    };
    this.shouldRender = (prev, next) => {
    ...
    this.transformSingle = (shape, bounds, { initialShape, scaleX, scaleY }) => {
      ...
      return {
        point: import_vec12.Vec.toFixed([bounds.minX, bounds.minY]),
        style: __spreadProps(__spreadValues({}, initialShape.style), {
    ...
    this.onDoubleClickBoundsHandle = (shape) => {
      ...
      return {
        ...
        point: import_vec12.Vec.toFixed(import_vec12.Vec.add(shape.point, import_vec12.Vec.sub(center, newCenter)))
      };
    };
    this.getSvgElement = (shape) => {
      const bounds = this.getBounds(shape);
...
var import_intersect7 = __toModule(require("@tldraw/intersect"));
...
function getFillPath(shape) {
  if (shape.points.length < 2)
    return "";
  return import_core13.Utils.getSvgPathFromStroke((0, import_perfect_freehand5.getStrokePoints)(shape.points, getFreehandOptions(shape)).map((pt) => pt.point));
}
...
function getDrawStrokePathTDSnapshot(shape) {
  ...
  const stroke = (0, import_perfect_freehand5.getStrokeOutlinePoints)(strokePoints, options2);
  const path = import_core13.Utils.getSvgPathFromStroke(stroke);
  return path;
}
function getSolidStrokePathTDSnapshot(shape) {
  ...
  const path = import_core13.Utils.getSvgPathFromStroke(strokePoints, false);
  return path;
}
...
var DrawUtil = class extends TDShapeUtil {
  constructor() {
    ...
    this.Component = TDShapeUtil.Component(({ shape, meta, isSelected, isGhost, events }, ref) => {
      ...
      const polygonPathTDSnapshot = React9.useMemo(() => {
        return getFillPath(shape);
      }, [points, style.size]);
      const pathTDSnapshot = React9.useMemo(() => {
      ...
      if (verySmall) {
        ...
        return /* @__PURE__ */ React9.createElement(import_core14.SVGContainer, __spreadValues({
          ref,
          id: shape.id + "_svg"
        }, events), /* @__PURE__ */ React9.createElement("circle", {
          r: sw2,
          fill: stroke,
          stroke,
          pointerEvents: "all",
      ...
      const shouldFill = style.isFilled && points.length > 3 && import_vec13.Vec.dist(points[0], points[points.length - 1]) < strokeWidth * 2;
      if (shape.style.dash === DashStyle.Draw) {
        return /* @__PURE__ */ React9.createElement(import_core14.SVGContainer, __spreadValues({
          ref,
          id: shape.id + "_svg"
        }, events), /* @__PURE__ */ React9.createElement("g", {
          opacity: isGhost ? GHOSTED_OPACITY : 1
        }, /* @__PURE__ */ React9.createElement("path", {
          className: shouldFill || isSelected ? "tl-fill-hitarea" : "tl-stroke-hitarea",
          d: pathTDSnapshot
        }), shouldFill && /* @__PURE__ */ React9.createElement("path", {
    ...
    this.Indicator = TDShapeUtil.Indicator(({ shape }) => {
      ...
      if (verySmall) {
        return /* @__PURE__ */ React9.createElement("circle", {
          x: bounds.width / 2,
          y: bounds.height / 2,
          r: 1
        });
    ...
    this.transform = (shape, bounds, { initialShape, scaleX, scaleY }) => {
      const initialShapeBounds = import_core14.Utils.getFromCache(this.boundsCache, initialShape, () => import_core14.Utils.getBoundsFromPoints(initialShape.points));
      ...
      const newBounds = import_core14.Utils.getBoundsFromPoints(shape.points);
      const point = import_vec13.Vec.sub([bounds.minX, bounds.minY], [newBounds.minX, newBounds.minY]);
      return {
        points,
        point
      };
    ...
    this.getBounds = (shape) => {
      ...
      if (pointsHaveChanged) {
        const bounds = import_core14.Utils.getBoundsFromPoints(shape.points);
        this.pointsBoundsCache.set(shape.points, bounds);
        this.shapeBoundsCache.set(shape.id, import_core14.Utils.translateBounds(bounds, shape.point));
        this.pointCache[shape.id] = shape.point;
      } else if (pointHasChanged && !pointsHaveChanged) {
        this.pointCache[shape.id] = shape.point;
        this.shapeBoundsCache.set(shape.id, import_core14.Utils.translateBounds(this.pointsBoundsCache.get(shape.points), shape.point));
      }
    ...
    this.hitTestPoint = (shape, point) => {
      const ptA = import_vec13.Vec.sub(point, shape.point);
      return import_core14.Utils.pointInPolyline(ptA, shape.points);
    };
    this.hitTestLineSegment = (shape, A, B) => {
      const { points, point } = shape;
      const ptA = import_vec13.Vec.sub(A, point);
      const ptB = import_vec13.Vec.sub(B, point);
      const bounds = this.getBounds(shape);
      if (points.length <= 2) {
        return import_vec13.Vec.distanceToLineSegment(A, B, shape.point) < 4;
      }
    ...
    this.hitTestBounds = (shape, bounds) => {
      if (!shape.rotation) {
        const shapeBounds = this.getBounds(shape);
        return import_core14.Utils.boundsContain(bounds, shapeBounds) || (import_core14.Utils.boundsContain(shapeBounds, bounds) || (0, import_intersect7.intersectBoundsBounds)(shapeBounds, bounds).length > 0) && (0, import_intersect7.intersectBoundsPolyline)(import_core14.Utils.translateBounds(bounds, import_vec13.Vec.neg(shape.point)), shape.points).length > 0;
      }
      const rBounds = this.getRotatedBounds(shape);
      const rotatedBounds = import_core14.Utils.getFromCache(this.rotatedCache, shape, () => {
        const c = import_core14.Utils.getBoundsCenter(import_core14.Utils.getBoundsFromPoints(shape.points));
        return shape.points.map((pt) => import_vec13.Vec.rotWith(pt, c, shape.rotation || 0));
      });
      return import_core14.Utils.boundsContain(bounds, rBounds) || (0, import_intersect7.intersectBoundsPolyline)(import_core14.Utils.translateBounds(bounds, import_vec13.Vec.neg(shape.point)), rotatedBounds).length > 0;
    };
  }
};
...
var _TLDR = class {
  ...
  static screenToWorld(data, point) {
    ...
    return import_vec14.Vec.sub(import_vec14.Vec.div(point, camera.zoom), camera.point);
  }
  ...
  static getRotatedShapeMutation(shape, center, origin, delta) {
    var _a, _b;
    const relativeCenter = import_vec14.Vec.sub(center, shape.point);
    const rotatedCenter = import_vec14.Vec.rotWith(center, origin, delta);
    const nextPoint = import_vec14.Vec.toFixed(import_vec14.Vec.sub(rotatedCenter, relativeCenter));
    if (shape.handles !== void 0) {
      const change = (_b = (_a = this.getShapeUtil(shape)).onHandleChange) == null ? void 0 : _b.call(_a, __spreadProps(__spreadValues({}, shape), { point: nextPoint }), Object.fromEntries(Object.entries(shape.handles).map(([handleId, handle]) => {
        const point = import_vec14.Vec.toFixed(import_vec14.Vec.rotWith(handle.point, relativeCenter, delta));
...
function alignShapes(app, ids, type) {
  ...
  initialShapes.forEach((shape) => {
    if (shape.type === TDShapeType.Group) {
      const delta = import_vec15.Vec.sub(after[shape.id].point, before[shape.id].point);
      shape.children.forEach((id) => {
        const child = app.getShape(id);
        before[child.id] = { point: child.point };
        after[child.id] = { point: import_vec15.Vec.add(child.point, delta) };
      });
...
function distributeShapes(app, ids, type) {
  ...
  initialShapes.forEach((shape) => {
    if (shape.type === TDShapeType.Group) {
      const delta = import_vec16.default.sub(after[shape.id].point, before[shape.id].point);
      shape.children.forEach((id) => {
        const child = app.getShape(id);
        before[child.id] = { point: child.point };
        after[child.id] = { point: import_vec16.default.add(child.point, delta) };
      });
      delete before[shape.id];
      delete after[shape.id];
    }
...
function duplicateShapes(app, ids, point) {
  ...
  if (point) {
    ...
    shapesToMove.forEach((shape) => {
      if (!shape.point)
        return;
      shape.point = import_vec17.Vec.sub(point, import_vec17.Vec.sub(center, shape.point));
    });
  } else {
    const offset = [16, 16];
    shapesToMove.forEach((shape) => {
      if (!shape.point)
        return;
      shape.point = import_vec17.Vec.add(shape.point, offset);
    });
  }
  shapesToMove.forEach((shape) => {
    if (shape.isLocked) {
...
function moveShapesToPage(app, ids, viewportBounds, fromPageId, toPageId) {
  ...
  const point = import_vec18.Vec.toFixed(import_vec18.Vec.add([-bounds.minX, -bounds.minY], [mx, my]));
  return {
    id: "move_to_page",
    before: {
      appState: {
...
function styleShapes(app, ids, changes) {
  ...
  shapeIdsToMutate.map((id) => app.getShape(id)).filter((shape) => !shape.isLocked).forEach((shape) => {
    ...
    if (shape.type === TDShapeType.Text) {
      ...
      afterShapes[shape.id].point = import_vec19.Vec.toFixed(import_vec19.Vec.add(shape.point, import_vec19.Vec.sub(app.getShapeUtil(shape).getCenter(shape), app.getShapeUtil(shape).getCenter(__spreadProps(__spreadValues({}, shape), {
...
function translateShapes(app, ids, delta) {
  ...
  const change = TLDR.mutateShapes(app.state, idsToMutate, (shape) => ({
    point: import_vec20.Vec.toFixed(import_vec20.Vec.add(shape.point, delta))
  }), currentPageId);
  ...
  constructor(app, shapeId, handleId, isCreate = false) {
    ...
    this.update = () => {
      ...
      if (!handles[handleId].canBind)
        return;
      let delta = import_vec21.Vec.sub(currentPoint, handles[handleId].point);
      if (shiftKey) {
        const A = handles[handleId === "start" ? "end" : "start"].point;
        const B = handles[handleId].point;
        const C = import_vec21.Vec.toFixed(import_vec21.Vec.sub(import_vec21.Vec.add(B, delta), shape.point));
        const angle = import_vec21.Vec.angle(A, C);
        const adjusted = import_vec21.Vec.rotWith(C, A, import_core26.Utils.snapAngleToSegments(angle, 24) - angle);
        delta = import_vec21.Vec.add(delta, import_vec21.Vec.sub(adjusted, C));
      }
      const nextPoint = import_vec21.Vec.sub(import_vec21.Vec.add(handles[handleId].point, delta), shape.point);
      const handle = __spreadProps(__spreadValues({}, handles[handleId]), {
        point: showGrid ? import_vec21.Vec.snap(nextPoint, currentGrid) : import_vec21.Vec.toFixed(nextPoint),
        bindingId: void 0
      });
      ...
      if (this.startBindingShapeId) {
        ...
        if (!metaKey) {
          ...
          const rayPoint = import_vec21.Vec.add(handle2.point, next.shape.point);
          const rayOrigin = center;
          const rayDirection = import_vec21.Vec.uni(import_vec21.Vec.sub(rayPoint, rayOrigin));
      ...
      if (!metaKey) {
        ...
        const rayOrigin = import_vec21.Vec.add(oppositeHandle.point, next.shape.point);
        const rayPoint = import_vec21.Vec.add(handle2.point, next.shape.point);
        const rayDirection = import_vec21.Vec.uni(import_vec21.Vec.sub(rayPoint, rayOrigin));
    ...
    this.complete = () => {
      ...
      if (!(currentBindingId || initialBinding) && import_vec21.Vec.dist(currentShape.handles.start.point, currentShape.handles.end.point) < 4) {
        return this.cancel();
      }
      const beforeBindings = {};
      const afterBindings = {};
    ...
    this.findBindingPoint = (shape, target, handleId, bindingId, point, origin, direction, bindAnywhere) => {
      ...
      return {
        ...
        toId: target.id,
        handleId,
        point: import_vec21.Vec.toFixed(bindingPoint.point),
...
var DrawSession = class extends BaseSession {
  constructor(app, id) {
    ...
    this.update = () => {
      ...
      const newAdjustedPoint = import_vec22.Vec.toFixed(import_vec22.Vec.sub(currentPoint, originPoint)).concat(currentPoint[2]);
      if (import_vec22.Vec.isEqual(this.lastAdjustedPoint, newAdjustedPoint))
        return;
      ...
      const delta = import_vec22.Vec.sub(topLeft, originPoint);
      let points;
      if (prevTopLeft[0] !== topLeft[0] || prevTopLeft[1] !== topLeft[1]) {
        this.topLeft = topLeft;
        points = this.points.map((pt) => {
          return import_vec22.Vec.toFixed(import_vec22.Vec.sub(pt, delta)).concat(pt[2]);
        });
      } else {
        points = [...this.shiftedPoints, import_vec22.Vec.sub(newAdjustedPoint, delta).concat(newAdjustedPoint[2])];
      }
              ...
              [pageId]: {
                shapes: {
                  [shapeId]: __spreadProps(__spreadValues({}, shape), {
                    point: import_vec22.Vec.toFixed(shape.point),
                    points: shape.points.map((pt) => import_vec22.Vec.toFixed(pt)),
                    isComplete: true
                  })
                }
              }
...
var HandleSession = class extends BaseSession {
  constructor(app, shapeId, handleId, commandId = "move_handle") {
    ...
    this.update = () => {
      ...
      const handles = shape.handles;
      const handleId = this.handleId;
      const delta = import_vec23.Vec.sub(currentPoint, handles[handleId].point);
      const handle = __spreadProps(__spreadValues({}, handles[handleId]), {
        point: import_vec23.Vec.sub(import_vec23.Vec.add(handles[handleId].point, delta), shape.point)
      });
...
var RotateSession = class extends BaseSession {
  constructor(app) {
    ...
    this.update = () => {
      ...
      const shapes = {};
      let directionDelta = import_vec24.Vec.angle(commonBoundsCenter, currentPoint) - this.initialAngle;
      if (shiftKey) {
    ...
    this.initialAngle = import_vec24.Vec.angle(this.commonBoundsCenter, originPoint);
  }
};
    ...
    this.update = () => {
      ...
      if ((isSnapping && !metaKey || !isSnapping && metaKey) && this.speed * camera.zoom < SLOW_SPEED && this.snapInfo.state === "ready") {
        ...
        if (snapResult) {
          ...
          newBounds = import_core30.Utils.getTransformedBoundingBox(initialCommonBounds, transformType, import_vec25.Vec.sub(delta, snapResult.offset), 0, shiftKey || isAllAspectRatioLocked);
        }
      }
  ...
  constructor(app, id, transformType, isCreate = false) {
    ...
    this.update = () => {
      ...
      if (initialShape.isLocked)
        return void 0;
      const shapes = {};
      const delta = altKey ? import_vec26.Vec.mul(import_vec26.Vec.sub(currentPoint, originPoint), 2) : import_vec26.Vec.sub(currentPoint, originPoint);
      const shape = this.app.getShape(initialShape.id);
      const utils = TLDR.getShapeUtil(shape);
      ...
      if ((isSnapping && !metaKey || !isSnapping && metaKey) && !initialShape.rotation && this.speed * camera.zoom < SLOW_SPEED && this.snapInfo.state === "ready") {
        ...
        if (snapResult) {
          snapLines = snapResult.snapLines;
          newBounds = import_core31.Utils.getTransformedBoundingBox(initialShapeBounds, transformType, import_vec26.Vec.sub(delta, snapResult.offset), shape.rotation, shiftKey || shape.isAspectRatioLocked || utils.isAspectRatioLocked);
        }
      }
      ...
      if (showGrid && afterShape && afterShape.point) {
        afterShape.point = import_vec26.Vec.snap(afterShape.point, currentGrid);
      }
      return {
        appState: {
          snapLines
...
var TranslateSession = class extends BaseSession {
  constructor(app, isCreate = false, link = false) {
    ...
    this.update = () => {
      ...
      const nextShapes = {};
      const nextPageState = {};
      let delta = import_vec27.Vec.sub(currentPoint, originPoint);
      ...
      const speed = import_vec27.Vec.dist(currentPoint, previousPoint);
      ...
      if ((isSnapping && !metaKey || !isSnapping && metaKey) && this.speed * camera.zoom < SLOW_SPEED && this.snapInfo.state === "ready") {
        ...
        if (snapResult) {
          ...
          delta = import_vec27.Vec.sub(delta, snapResult.offset);
        }
      }
      this.prev = delta;
      if (this.isCloning) {
        if (didChangeCloning) {
          ...
          clones.forEach((clone) => {
            nextShapes[clone.id] = __spreadProps(__spreadValues({}, clone), {
              point: showGrid ? import_vec27.Vec.snap(import_vec27.Vec.toFixed(import_vec27.Vec.add(clone.point, delta)), currentGrid) : import_vec27.Vec.toFixed(import_vec27.Vec.add(clone.point, delta))
            });
          });
        } else {
          if (this.cloneInfo.state === "empty")
        ...
        if (didChangeCloning) {
          ...
          initialShapes.forEach((shape) => {
            nextShapes[shape.id] = {
              point: showGrid ? import_vec27.Vec.snap(import_vec27.Vec.toFixed(import_vec27.Vec.add(shape.point, delta)), currentGrid) : import_vec27.Vec.toFixed(import_vec27.Vec.add(shape.point, delta))
            };
          });
          for (const binding of clonedBindings) {
...
var EraseSession = class extends BaseSession {
  constructor(app) {
    ...
    this.update = () => {
      ...
      if (shiftKey) {
        if (!this.isLocked && import_vec28.Vec.dist(originPoint, currentPoint) > 4) {
          if (!this.lockedDirection) {
            const delta = import_vec28.Vec.sub(currentPoint, originPoint);
      ...
      const newPoint = import_vec28.Vec.toFixed(import_vec28.Vec.add(originPoint, import_vec28.Vec.sub(currentPoint, originPoint)));
...
var GridSession = class extends BaseSession {
  constructor(app, id) {
    ...
    this.update = () => {
      ...
      const offset = import_vec29.Vec.sub(currentPoint, center);
      if (shiftKey) {
      ...
      for (let x = minX; x < maxX; x++) {
        for (let y = minY; y < maxY; y++) {
          ...
          if (x === 0 && y === 0)
            continue;
          const clone = this.getClone(import_vec29.Vec.add(this.shape.point, [x * gapX, y * gapY]), isCopying);
  ...
  constructor() {
    ...
    this.getShapeClone = (id, side) => {
      ...
      if (utils.canClone) {
        ...
        if (shape.rotation !== 0) {
          const newCenter = import_vec30.default.add(point, [bounds.width / 2, bounds.height / 2]);
          const rotatedCenter = import_vec30.default.rotWith(newCenter, center, shape.rotation || 0);
          point = import_vec30.default.sub(rotatedCenter, [bounds.width / 2, bounds.height / 2]);
        }
    ...
    this.onPointerMove = (info, e) => {
      ...
      if (this.status === Status2.SpacePanning && e.buttons === 1 || this.status === Status2.MiddleWheelPanning && e.buttons === 4) {
        (_b = (_a = this.app).onPan) == null ? void 0 : _b.call(_a, __spreadProps(__spreadValues({}, info), { delta: import_vec30.default.neg(info.delta) }), e);
        return;
      }
      if (this.status === Status2.PointingBoundsHandle) {
        if (!this.pointedBoundsHandle)
          throw Error("No pointed bounds handle");
        if (import_vec30.default.dist(originPoint, currentPoint) > DEAD_ZONE) {
    ...
    this.onPointerMove = (info) => {
      switch (this.status) {
        case Status3.Pointing: {
          if (import_vec31.default.dist(info.origin, info.point) > DEAD_ZONE) {
  ...
  constructor() {
    ...
    this.onPointerDown = () => {
      ...
      if (this.status === Status.Idle) {
        ...
        this.app.createTextShapeAtPoint(showGrid ? import_vec32.default.snap(currentPoint, currentGrid) : currentPoint);
...
var import_vec38 = __toModule(require("@tldraw/vec"));
  ...
  constructor() {
    ...
    this.onPointerDown = () => {
      ...
      if (this.status === Status.Idle) {
        ...
        newShape.point = import_vec38.default.sub(newShape.point, [bounds.width / 2, bounds.height / 2]);
...
var StateManager = class {
  constructor(initialState, id, version, update) {
    ...
    this.applyPatch = (patch, id) => {
      ...
      const next = import_core43.Utils.deepMerge(this._state, patch);
...
// src/state/TldrawApp.ts
var uuid = import_core44.Utils.uniqueId();
var _TldrawApp = class extends StateManager {
  constructor(id, callbacks = {}) {
    ...
    this.viewport = import_core44.Utils.getBoundsFromPoints([
      [0, 0],
      [100, 100]
    ]);
    this.rendererBounds = import_core44.Utils.getBoundsFromPoints([
      [0, 0],
      [100, 100]
    ]);
    this.selectHistory = {
        ...
        Object.entries(next.document.pages).forEach(([pageId, page]) => {
          ...
          if (!prevPage || page.shapes !== prevPage.shapes || page.bindings !== prevPage.bindings) {
            ...
            groupsToUpdate.forEach((group) => {
              ...
              const commonBounds = import_core44.Utils.getCommonBounds(children.map((id) => page.shapes[id]).filter(Boolean).map((shape) => TLDR.getRotatedBounds(shape)));
    ...
    this.replacePageContent = (shapes, bindings, pageId = this.currentPageId) => {
      ...
      this.useStore.setState((current) => {
        ...
        Object.values(nextShapes).forEach((shape) => {
          ...
          const commonBounds = import_core44.Utils.getCommonBounds(children.map((id) => page.shapes[id]).filter(Boolean).map((shape2) => TLDR.getRotatedBounds(shape2)));
    ...
    this.updateViewport = (point, zoom) => {
      ...
      const [minX, minY] = import_vec39.Vec.sub(import_vec39.Vec.div([0, 0], zoom), point);
      const [maxX, maxY] = import_vec39.Vec.sub(import_vec39.Vec.div([width, height], zoom), point);
      this.viewport = {
        minX,
        minY,
        maxX,
    ...
    this.getPagePoint = (point, pageId = this.currentPageId) => {
      ...
      return import_vec39.Vec.sub(import_vec39.Vec.div(point, camera.zoom), camera.point);
    };
    this.createPage = (id) => {
      if (this.readOnly)
        return this;
    ...
    this.copy = (ids = this.selectedIds) => {
      ...
      const copyingShapes = copyingShapeIds.map((id) => import_core44.Utils.deepClone(this.getShape(id, this.currentPageId)));
    ...
    this.paste = (point) => {
      ...
      const pasteInCurrentPage = (shapes, bindings) => {
        const idsMap = {};
        shapes.forEach((shape) => idsMap[shape.id] = import_core44.Utils.uniqueId());
        bindings.forEach((binding) => idsMap[binding.id] = import_core44.Utils.uniqueId());
        ...
        const commonBounds = import_core44.Utils.getCommonBounds(shapesToPaste.map(TLDR.getBounds));
        let center = import_vec39.Vec.toFixed(this.getPagePoint(point || this.centerPoint));
        if (import_vec39.Vec.dist(center, this.pasteInfo.center) < 2 || import_vec39.Vec.dist(center, import_vec39.Vec.toFixed(import_core44.Utils.getBoundsCenter(commonBounds))) < 2) {
          center = import_vec39.Vec.add(center, this.pasteInfo.offset);
          this.pasteInfo.offset = import_vec39.Vec.add(this.pasteInfo.offset, [GRID_SIZE, GRID_SIZE]);
        } else {
          this.pasteInfo.center = center;
          this.pasteInfo.offset = [0, 0];
        }
        const centeredBounds = import_core44.Utils.centerBounds(commonBounds, center);
        const delta = import_vec39.Vec.sub(import_core44.Utils.getBoundsCenter(centeredBounds), import_core44.Utils.getBoundsCenter(commonBounds));
        this.create(shapesToPaste.map((shape) => TLDR.getShapeUtil(shape.type).create(__spreadProps(__spreadValues({}, shape), {
          point: import_vec39.Vec.toFixed(import_vec39.Vec.add(shape.point, delta)),
      ...
      try {
        ...
        navigator.clipboard.readText().then((result) => {
          try {
            ...
            TLDR.warn(e);
            const shapeId = import_core44.Utils.uniqueId();
            this.createShapes({
              id: shapeId,
              type: TDShapeType.Text,
    ...
    this.copySvg = (ids = this.selectedIds, pageId = this.currentPageId) => {
      ...
      const commonBounds = import_core44.Utils.getCommonBounds(shapes.map(TLDR.getRotatedBounds));
      const padding = 16;
    ...
    this.pan = (delta) => {
      ...
      return this.setCamera(import_vec39.Vec.toFixed(import_vec39.Vec.sub(camera.point, delta)), camera.zoom, `panned`);
    };
    this.pinchZoom = (point, delta, zoom) => {
      const { camera } = this.pageState;
      const nextPoint = import_vec39.Vec.sub(camera.point, import_vec39.Vec.div(delta, camera.zoom));
      const nextZoom = zoom;
      const p0 = import_vec39.Vec.sub(import_vec39.Vec.div(point, camera.zoom), nextPoint);
      const p1 = import_vec39.Vec.sub(import_vec39.Vec.div(point, nextZoom), nextPoint);
      return this.setCamera(import_vec39.Vec.toFixed(import_vec39.Vec.add(nextPoint, import_vec39.Vec.sub(p1, p0))), nextZoom, `pinch_zoomed`);
    };
    this.zoomTo = (next, center = this.centerPoint) => {
      const { zoom, point } = this.pageState.camera;
      const p0 = import_vec39.Vec.sub(import_vec39.Vec.div(center, zoom), point);
      const p1 = import_vec39.Vec.sub(import_vec39.Vec.div(center, next), point);
      return this.setCamera(import_vec39.Vec.toFixed(import_vec39.Vec.add(point, import_vec39.Vec.sub(p1, p0))), next, `zoomed_camera`);
    };
    this.zoomIn = () => {
    ...
    this.zoomToFit = () => {
      ...
      const commonBounds = import_core44.Utils.getCommonBounds(shapes.map(TLDR.getBounds));
      ...
      return this.setCamera(import_vec39.Vec.toFixed(import_vec39.Vec.sub([mx, my], [commonBounds.minX, commonBounds.minY])), zoom, `zoomed_to_fit`);
    };
    this.zoomToSelection = () => {
      ...
      return this.setCamera(import_vec39.Vec.toFixed(import_vec39.Vec.sub([mx, my], [selectedBounds.minX, selectedBounds.minY])), zoom, `zoomed_to_selection`);
    };
    this.zoomToContent = () => {
      const shapes = this.shapes;
      ...
      const mx = (rendererBounds.width - commonBounds.width * zoom) / 2 / zoom;
      const my = (rendererBounds.height - commonBounds.height * zoom) / 2 / zoom;
      return this.setCamera(import_vec39.Vec.toFixed(import_vec39.Vec.sub([mx, my], [commonBounds.minX, commonBounds.minY])), this.pageState.camera.zoom, `zoomed_to_content`);
    };
    this.resetZoom = () => {
      return this.zoomTo(1);
    };
    this.zoomBy = import_core44.Utils.throttle((delta, center) => {
    ...
    this.nudge = (delta, isMajor = false, ids = this.selectedIds) => {
      ...
      return this.setState(translateShapes(this, ids, import_vec39.Vec.mul(delta, size)));
    };
    ...
    this.group = (ids = this.selectedIds, groupId = import_core44.Utils.uniqueId(), pageId = this.currentPageId) => {
      if (this.readOnly)
        return this;
    ...
    this.onPan = (info, e) => {
      if (this.appState.status === "pinching")
        return;
      const delta = import_vec39.Vec.div(info.delta, this.pageState.camera.zoom);
      const prev = this.pageState.camera.point;
      const next = import_vec39.Vec.sub(prev, delta);
      if (import_vec39.Vec.isEqual(next, prev))
        return;
      this.pan(delta);
  ...
  createTextShapeAtPoint(point, id) {
    ...
    const newShape = Text2.create({
      id: id || import_core44.Utils.uniqueId(),
      parentId: currentPageId,
      childIndex,
      point,
      style: __spreadValues({}, currentStyle)
    ...
    newShape.point = import_vec39.Vec.sub(newShape.point, [bounds.width / 2, bounds.height / 2]);
  ...
  get centerPoint() {
    const { width, height } = this.rendererBounds;
    return import_vec39.Vec.toFixed([width / 2, height / 2]);
  }
  get currentGrid() {
    const { zoom } = this.pageState.camera;
    if (zoom < 0.15) {
...
var commandKey = () => import_core45.Utils.isDarwin() ? "\u2318" : "Ctrl";
function Kbd({
  variant,
  children
}) {
...
var shapeShapeIcons = {
  ...
  [TDShapeType.Triangle]: /* @__PURE__ */ React38.createElement(import_react_icons2.VercelLogoIcon, null),
...
function DMSubMenu({ children, disabled = false, label }) {
  return /* @__PURE__ */ React43.createElement(import_react_dropdown_menu8.Root, {
    dir: "ltr"
  }, /* @__PURE__ */ React43.createElement(import_react_dropdown_menu8.TriggerItem, {
    dir: "ltr",
    asChild: true
  }, /* @__PURE__ */ React43.createElement(RowButton, {
    disabled,
    hasArrow: true
  }, label)), /* @__PURE__ */ React43.createElement(import_react_dropdown_menu8.Content, {
    dir: "ltr",
    asChild: true,
    sideOffset: 2,
    alignOffset: -2
  }, /* @__PURE__ */ React43.createElement(MenuContent, null, children, /* @__PURE__ */ React43.createElement(import_react_dropdown_menu8.Arrow, {
    offset: 13
  }))));
}
...
function PreferencesMenu() {
  ...
  const toggleDebugMode = React49.useCallback(() => {
    app.setSetting("isDebugMode", (v) => !v);
  }, [app]);
  const toggleDarkMode = React49.useCallback(() => {
    app.setSetting("isDarkMode", (v) => !v);
  }, [app]);
  const toggleFocusMode = React49.useCallback(() => {
    app.setSetting("isFocusMode", (v) => !v);
  }, [app]);
  const toggleRotateHandle = React49.useCallback(() => {
    app.setSetting("showRotateHandles", (v) => !v);
  }, [app]);
  const toggleGrid = React49.useCallback(() => {
    app.setSetting("showGrid", (v) => !v);
  }, [app]);
  const toggleBoundShapesHandle = React49.useCallback(() => {
    app.setSetting("showBindingHandles", (v) => !v);
  }, [app]);
  const toggleisSnapping = React49.useCallback(() => {
    app.setSetting("isSnapping", (v) => !v);
  }, [app]);
  const toggleCloneControls = React49.useCallback(() => {
    app.setSetting("showCloneHandles", (v) => !v);
  }, [app]);
  return /* @__PURE__ */ React49.createElement(DMSubMenu, {
    label: "Preferences"
  }, /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
    checked: settings.isDarkMode,
    onCheckedChange: toggleDarkMode,
    kbd: "#\u21E7D"
  }, "Dark Mode"), /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
    checked: settings.isFocusMode,
    onCheckedChange: toggleFocusMode,
    kbd: "#."
  }, "Focus Mode"), /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
    checked: settings.isDebugMode,
    onCheckedChange: toggleDebugMode
  }, "Debug Mode"), /* @__PURE__ */ React49.createElement(DMDivider, null), /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
    checked: settings.showRotateHandles,
    onCheckedChange: toggleRotateHandle
  }, "Rotate Handles"), /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
    checked: settings.showBindingHandles,
    onCheckedChange: toggleBoundShapesHandle
  }, "Binding Handles"), /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
    checked: settings.showCloneHandles,
    onCheckedChange: toggleCloneControls
  }, "Clone Handles"), /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
    checked: settings.showGrid,
    onCheckedChange: toggleGrid,
    kbd: "#\u21E7G"
  }, "Grid"), /* @__PURE__ */ React49.createElement(DMCheckboxItem, {
...
var Menu = React51.memo(function Menu2({ showSponsorLink, readOnly }) {
  ...
  return /* @__PURE__ */ React51.createElement(DropdownMenu3.Root, {
    ...
    variant: "menu"
  }, showFileMenu && /* @__PURE__ */ React51.createElement(DMSubMenu, {
    label: "File..."
  }, app.callbacks.onNewProject && /* @__PURE__ */ React51.createElement(DMItem, {
    onClick: onNewProject,
    kbd: "#N"
  }, "New Project"), app.callbacks.onOpenProject && /* @__PURE__ */ React51.createElement(DMItem, {
    onClick: onOpenProject,
    kbd: "#O"
  }, "Open..."), app.callbacks.onSaveProject && /* @__PURE__ */ React51.createElement(DMItem, {
    onClick: onSaveProject,
    kbd: "#S"
  }, "Save"), app.callbacks.onSaveProjectAs && /* @__PURE__ */ React51.createElement(DMItem, {
    onClick: onSaveProjectAs,
    kbd: "#\u21E7S"
  }, "Save As...")), !readOnly && /* @__PURE__ */ React51.createElement(React51.Fragment, null, /* @__PURE__ */ React51.createElement(DMSubMenu, {
    label: "Edit..."
  }, /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    onClick: app.undo,
    kbd: "#Z"
  }, "Undo"), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    onClick: app.redo,
    kbd: "#\u21E7Z"
  }, "Redo"), /* @__PURE__ */ React51.createElement(DMDivider, {
    dir: "ltr"
  }), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    disabled: !hasSelection,
    onClick: handleCut,
    kbd: "#X"
  }, "Cut"), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    disabled: !hasSelection,
    onClick: handleCopy,
    kbd: "#C"
  }, "Copy"), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    onClick: handlePaste,
    kbd: "#V"
  }, "Paste"), /* @__PURE__ */ React51.createElement(DMDivider, {
    dir: "ltr"
  }), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    disabled: !hasSelection,
    ...
    kbd: "#\u21E7C"
  }, "Copy as SVG"), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    disabled: !hasSelection,
    onClick: handleCopyJson
  }, "Copy as JSON"), /* @__PURE__ */ React51.createElement(DMDivider, {
    dir: "ltr"
  }), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    onClick: handleSelectAll,
    kbd: "#A"
  }, "Select All"), /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: preventEvent,
    onClick: handleselectNone
  }, "Select None"))), /* @__PURE__ */ React51.createElement("a", {
    href: "https://tldraw.com/r"
  }, /* @__PURE__ */ React51.createElement(DMItem, null, "Create a Multiplayer Room")), /* @__PURE__ */ React51.createElement(DMDivider, {
    dir: "ltr"
  }), /* @__PURE__ */ React51.createElement(PreferencesMenu, null), /* @__PURE__ */ React51.createElement(DMDivider, {
    dir: "ltr"
  }), /* @__PURE__ */ React51.createElement("a", {
    href: "https://github.com/Tldraw/Tldraw",
    target: "_blank",
    rel: "nofollow"
  }, /* @__PURE__ */ React51.createElement(DMItem, null, "GitHub", /* @__PURE__ */ React51.createElement(SmallIcon, null, /* @__PURE__ */ React51.createElement(import_react_icons5.GitHubLogoIcon, null)))), /* @__PURE__ */ React51.createElement("a", {
    href: "https://twitter.com/Tldraw",
    target: "_blank",
    rel: "nofollow"
  }, /* @__PURE__ */ React51.createElement(DMItem, null, "Twitter", /* @__PURE__ */ React51.createElement(SmallIcon, null, /* @__PURE__ */ React51.createElement(import_react_icons5.TwitterLogoIcon, null)))), /* @__PURE__ */ React51.createElement("a", {
    href: "https://discord.gg/SBBEVCA4PG",
    target: "_blank",
    rel: "nofollow"
  }, /* @__PURE__ */ React51.createElement(DMItem, null, "Discord", /* @__PURE__ */ React51.createElement(SmallIcon, null, /* @__PURE__ */ React51.createElement(DiscordIcon, null)))), showSponsorLink && /* @__PURE__ */ React51.createElement("a", {
    href: "https://github.com/sponsors/steveruizok",
    target: "_blank",
    rel: "nofollow"
  }, /* @__PURE__ */ React51.createElement(DMItem, {
    isSponsor: true
  }, "Become a Sponsor", " ", /* @__PURE__ */ React51.createElement(SmallIcon, null, /* @__PURE__ */ React51.createElement(HeartIcon, null)))), showSignInOutMenu && /* @__PURE__ */ React51.createElement(React51.Fragment, null, /* @__PURE__ */ React51.createElement(DMDivider, {
    dir: "ltr"
  }), " ", app.callbacks.onSignIn && /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: handleSignIn
  }, "Sign In"), app.callbacks.onSignOut && /* @__PURE__ */ React51.createElement(DMItem, {
    onSelect: handleSignOut
  }, "Sign Out", /* @__PURE__ */ React51.createElement(SmallIcon, null, /* @__PURE__ */ React51.createElement(import_react_icons5.ExitIcon, null))))));
});
...
function Tldraw({
  ...
  return /* @__PURE__ */ React59.createElement(TldrawContext.Provider, {
    value: app
  }, /* @__PURE__ */ React59.createElement(InnerTldraw, {
    key: sId || "Tldraw",
    id: sId,
    autofocus,
    showPages,
...
var InnerTldraw = React59.memo(function InnerTldraw2({
  ...
  }, /* @__PURE__ */ React59.createElement(import_core46.Renderer, {
    id,
